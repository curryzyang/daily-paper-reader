...
score: 10.0
evidence: 先验引导的符号回归用于方程发现中的科学一致性
tldr: 符号回归旨在从数据中发现可解释方程，但现有方法常陷入“伪方程陷阱”，即拟合良好却违背科学原理。本文提出PG-SR框架，通过预热、进化和精炼三阶段流程，引入先验约束检查器和先验退火约束评估机制（PACE），将领域先验显式编码为约束。理论证明该方法能降低假设空间复杂度并提升泛化能力，实验表明其在多领域优于现有方法，且对噪声和数据稀缺具有鲁棒性。
selection_source: fresh_fetch
motivation: 现有符号回归方法过度依赖经验风险最小化，导致发现的方程虽拟合数据但缺乏科学一致性。
method: 提出PG-SR框架，利用先验约束检查器和先验退火约束评估机制（PACE），在进化过程中引导方程发现符合科学先验。
result: 实验证明PG-SR在多个领域均优于最先进的基准模型，并在噪声数据和样本稀缺情况下表现出极强的鲁棒性。
conclusion: 通过引入显式先验约束，PG-SR有效解决了伪方程问题，为发现具有科学一致性的物理定律提供了可靠途径。
---
