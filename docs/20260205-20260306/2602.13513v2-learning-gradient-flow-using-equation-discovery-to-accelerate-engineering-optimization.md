...
tldr: 本研究针对工程优化中目标函数及其梯度评估成本高昂的问题，提出了一种基于数据驱动方程发现的“学习梯度流”（LGF）优化器。该方法通过分析梯度下降、牛顿法或ADAM等算法的优化轨迹，学习其连续时间动力学特征，并构建全维或降维空间的代理模型。LGF能够捕捉优化路径的关键特征，在减少昂贵计算的同时显著加速收敛，为结构拓扑优化和逆问题等复杂工程挑战提供了高效的解决方案。
selection_source: fresh_fetch
motivation: 旨在解决传统优化算法在处理复杂工程问题时，因频繁评估高成本目标函数和梯度而导致的计算效率低下问题。
method: 引入LGF优化器，利用方程发现技术从优化变量的轨迹数据中学习梯度流动力学，并将其作为代理模型进行快速求解。
result: 在逆问题、结构拓扑优化及科学机器学习等多个标准工程案例中，该方法成功捕捉了优化轨迹特征并大幅提升了收敛效率。
conclusion: 学习梯度流动力学是一种加速工程优化的有效手段，能够在保证精度的前提下显著降低计算负担。
---
