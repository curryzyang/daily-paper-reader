...
tags: ["query:sr"]
score: 9.0
evidence: 使用KAN发现混沌和复杂动力系统的控制方程
tldr: 本研究提出 KANDy，一种基于 Kolmogorov-Arnold 网络（KAN）的零深度、宽架构神经网络，旨在发现混沌及复杂动力系统的控制方程。KANDy 通过将 KAN 与方程发现相结合，替代了传统的稀疏回归方法，有效解决了稀疏性约束带来的局限。该模型在离散、连续系统及混沌偏微分方程中表现出色，并能恢复 Hopf 纤维化的拓扑结构，为非线性动力系统建模提供了一种兼具解释性和有效性的新工具。
selection_source: fresh_fetch
motivation: 旨在克服传统稀疏回归在方程发现中的稀疏性限制，并解决 KAN 在处理动力系统时的局限性。
method: 引入 KANDy 架构，利用零深度且宽的 KAN 结构来显式学习并提取动力系统的控制方程。
result: 实验证明该模型能准确识别离散、连续系统及偏微分方程的控制律，并能保持吸引子的拓扑一致性。
conclusion: KANDy 是一种强有力的可解释建模工具，为复杂非线性动力系统的自动化发现提供了新途径。
---
