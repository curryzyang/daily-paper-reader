...
score: 8.0
evidence: 使用进化搜索和假设管理的自动化科学发现框架
tldr: OR-Agent是一个多智能体研究框架，旨在自动化复杂实验领域的科学发现。它将研究组织为结构化的树状工作流，结合了进化搜索与系统化假设管理。通过引入分层反射机制（模拟梯度、动量和权重衰减），该框架能有效管理研究轨迹并进行深度探索。在组合优化和自动驾驶模拟实验中，OR-Agent表现优于传统进化基准，为AI辅助科学发现提供了可扩展且可检查的方案。
